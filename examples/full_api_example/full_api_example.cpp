...
// some forward references to keep the compiler happy
...
static int usage()
{
  fprintf(stdout, "full_api_example -b [uartBaudRate] -D [uartDevicePath]\n");
  return -1;
}
...
int main(int argc, char *argv[])
{
  ...
  af_transport_t *platformUART = linux_uart_create(uart_device_path, baud_rate);
  ...
  while (1) {
    ...
    // We can talk to the ASR from here on down. First, some simple housekeeping

    // If we need to reboot the ASR (e.g. after an OTA firmware update), do it now.
    if (asrRebootPending) {
      rebootAsr();  // reboot cleanly, don't force it
    ...
    if (lastStep == currStep) continue; // At this point we're still waiting on the last test to complete.
