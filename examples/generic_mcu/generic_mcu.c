...
static void handle_ota_info(const uint16_t value_len, const uint8_t *value) {
    ...
    switch (ota_info->state) {
        ...
        case AF_OTA_TRANSFER_BEGIN:
        {
            ...
            int res = af_lib_set_attribute_bytes(sAf_lib, AF_MCU_OTA_INFO, value_len, value, AF_LIB_SET_REASON_LOCAL_CHANGE);
            if (res != AF_SUCCESS) {
        ...
        case AF_OTA_APPLY:
            ...
            res = af_lib_set_attribute_64(sAf_lib, AF_MCU_TYPE_TO_VERSION_ATTRIBUTE(ota_state.begin_info.ota_type), ota_state.apply_info.version_id, AF_LIB_SET_REASON_LOCAL_CHANGE);
            if (res != AF_SUCCESS) {
...
static void handle_ota_transfer(const uint16_t value_len, const uint8_t *value) {
    ...
    int res = af_lib_set_attribute_bytes(sAf_lib, AF_MCU_OTA_TRANSFER, sizeof(result), result, AF_LIB_SET_REASON_LOCAL_CHANGE);
    if (res != AF_SUCCESS) {
    ...
    if (done) {
        ...
        res = af_lib_set_attribute_bytes(sAf_lib, AF_MCU_OTA_INFO, sizeof(ota_info), (uint8_t*)&ota_info, AF_LIB_SET_REASON_LOCAL_CHANGE);
        if (res != AF_SUCCESS) {
            fprintf(stderr, "handle_ota_transfer: error setting attribute %u, %d\n", AF_MCU_OTA_INFO, res);
        }
    }
        ...
        case AF_SYSTEM_ASR_STATE:
            ...
            switch (value[0]) {
                ...
                case AF_MODULE_STATE_UPDATE_READY:
                    ...
                    while (af_lib_set_attribute_32(sAf_lib, AF_SYSTEM_COMMAND, AF_MODULE_COMMAND_REBOOT, AF_LIB_SET_REASON_LOCAL_CHANGE) != AF_SUCCESS) {
                        af_lib_loop(sAf_lib);
                    }
                    break;
...
static void get_response_handler_event(const af_lib_error_t error, const uint16_t attribute_id, const uint16_t value_len, const uint8_t *value) {
    printAttribute("AF_LIB_EVENT_GET_RESPONSE", attribute_id, value_len, value, error);
...
void attr_event_callback(const af_lib_event_type_t event_type, const af_lib_error_t error, const uint16_t attribute_id, const uint16_t value_len, const uint8_t *value) {
    switch (event_type) {
        ...
        case AF_LIB_EVENT_MCU_GET_REQUEST:
            printAttribute("AF_LIB_EVENT_MCU_GET_REQUEST", attribute_id, value_len, value, error);
            break;
        case AF_LIB_EVENT_GET_RESPONSE:         // Response to af_lib_get_attribute()
...
static void command_usage() {
    ...
    fprintf(stdout, "\tsetr <attribute id> <attribute type> <value in hex> (a set in response to ASR get request\n");
...
static void handle_command(char* buffer) {
    ...
    if (strncmp(token, "get", 3) == 0) {
        ...
        if (res != AF_SUCCESS) {
            ...
            strncmp(token, "setr", 4) == 0) {
        af_lib_set_reason_t set_reason = AF_LIB_SET_REASON_LOCAL_CHANGE;
        if (strncmp(token, "setr", 4) == 0) {
            set_reason = AF_LIB_SET_REASON_GET_RESPONSE;
        }
        token = strsep(&buffer, " ");
        if (!token || !strlen(token)) {
            return command_usage();
        ...
        switch (attribute_type) {
            case ATTRIBUTE_TYPE_BOOLEAN:
                res = af_lib_set_attribute_bool(sAf_lib, attribute_id, attribute_value, set_reason);
                break;
            case ATTRIBUTE_TYPE_SINT8:
                res = af_lib_set_attribute_8(sAf_lib, attribute_id, attribute_value, set_reason);
                break;
            case ATTRIBUTE_TYPE_SINT16:
                res = af_lib_set_attribute_16(sAf_lib, attribute_id, attribute_value, set_reason);
                break;
            case ATTRIBUTE_TYPE_SINT32:
                res = af_lib_set_attribute_32(sAf_lib, attribute_id, attribute_value, set_reason);
                break;
            case ATTRIBUTE_TYPE_SINT64:
                res = af_lib_set_attribute_64(sAf_lib, attribute_id, attribute_value, set_reason);
                break;
            case ATTRIBUTE_TYPE_FIXED_16_16:
                // TODO, fix me...
                break;
            case ATTRIBUTE_TYPE_UTF8S:
                res = af_lib_set_attribute_str(sAf_lib, attribute_id, attribute_len, (char*)binary_attr_value, set_reason);
                break;
            case ATTRIBUTE_TYPE_BYTES:
                res = af_lib_set_attribute_bytes(sAf_lib, attribute_id, attribute_len, binary_attr_value, set_reason);
                break;
            default:
